...
ESX.RegisterServerCallback("s-inventory:getPlayerInventory",
...
RegisterServerEvent("s-inventory:tradePlayerItem")
AddEventHandler("s-inventory:tradePlayerItem", function(from, target, type, itemName, itemCount)
    local _source = from
...
RegisterCommand("openinventory", function(source, args, rawCommand)
    if IsPlayerAceAllowed(source, "inventory.openinventory") then
        ...
        if targetXPlayer ~= nil then
            TriggerClientEvent("s-inventory:openPlayerInventory", source, target, targetXPlayer.name)
        else
